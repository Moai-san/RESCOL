...
void ElitistSystem2::iterar()
{
    ...
    if (mejor_solucion.solucion_valida){
        for (auto &par : mejor_solucion.arcos_visitados_tour)
        {
            ...
            if (pasadas != 0)
                feromonas.at(a).cantidad += elitist*(tau / (rho * pow(mejor_solucion.longitud_camino_final, 2) * pasadas));
        }
        if (usarMatrizSecundaria)
        {
            for (auto &par : mejor_solucion.arcos_visitados_salida)
            {
                ...
                int pasadas = par.second;
                if (pasadas != 0)
                    feromonas_salida.at(a).cantidad += elitist*(tau / (rho * pow(mejor_solucion.longitud_camino_salida, 2) * pasadas));
            }
        }
    }
