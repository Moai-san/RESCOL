...
void ACO::construirSolucion(Hormiga &hormiga)
{
    ...
    while (!solucionCompleta(hormiga))
    {
        ...
        {
            buscarSalidaMatriz(hormiga);
        }
        if(usaDijkstra){
            buscarDijkstra(hormiga);
        }
...
Nodo *ACO::eligeSiguiente(Hormiga &hormiga)
{
    ...
    if(sin_nuevo >= LimiteDeMejoras && !hormiga.camino_tour.empty()){
        for (auto i : grafo->informacion_heuristica[hormiga.nodo_actual->id])
        {
            flag_falla = true;
...
void ACO::buscarSalidaMatriz(Hormiga &hormiga)
{
...
void ACO::buscarDijkstra(Hormiga &hormiga)
{
